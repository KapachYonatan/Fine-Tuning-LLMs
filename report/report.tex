...
\author{Yehonatan Kapach (318515756)}
\date{May 2026}
...
\textbf{DictaLM-2.0}---use \textbf{SentencePiece}, which treats the marker  \spmark{(U+2581)} as a literal whitespace placeholder. Reconstruction is straightforward in both cases through left-to-right concatenation. However, while the GPT-2 BPE marker dictates logical word boundaries that must be swapped for spaces, SentencePiece treats its marker as an actual character, allowing for a direct substitution back into standard whitespaces. 
...
The relevance score was determined using the "LLM as a judge" methodology; the outputs were prompted to a strong LLM (Claude sonnet 4.6), which was required to give a score to each response (Good/Partial/Poor).
